...
function NaklOnePage() {
   ...
   const onChangeDate = async (date) => {
      setDateOfAccept(date)
      setChangeDateToggle(!changeDateToggle)

      try {
         const responseNakl = await axios.get('/nakls-full/' + linkId, {
            headers: { accessToken: auth.token },
         })
         ...
         if (responseNakl.data.is_error || response.data.is_error) {
            setError(true)
         } else {
            responseNakl.data.data.doc_date = new Date(date).toISOString()
            response.data.data.doc_date = date
            setData(response.data.data)

            await axios.put(
               '/nakls-full/' + data.nakl_id,
               responseNakl.data.data,
               {
                  headers: { accessToken: auth.token },
               }
            )
         ...
         <div className="accent__block">
            ...
            <div className="accent__block-select">
               <div className="list-select">
                  ...
                  <SelectData onChange={onChangeDate} data={dateOfAccept} />
               </div>
               <div className="list-select">
