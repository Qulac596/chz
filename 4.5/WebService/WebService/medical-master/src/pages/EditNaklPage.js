...
function EditNaklPage() {
   ...
   useEffect(() => {
      ...
      async function fetchData() {
         try {
            ...
            const responseNakl = await axios.get(
               '/nakls-full/' + data.nakl_id,
               {
                  headers: { accessToken: auth.token },
               }
            )
            ...
            if (
               ...
               const responseData = responseNakl.data.data
               const newForm = { ...form }
               newForm.formControls.contractNumber.value =
                  responseData.contract_num
               newForm.formControls.invoiceNumber.value = responseData.doc_num
               setForm(newForm)
               const currentCompany = responseCompanies.data.data.find(
                  (item) => item.company_id === responseData.provider_id
               )
               const currentRecieveType = responseReceiveTypes.data.data.find(
                  (item) =>
                     item.receive_type_id === responseData.receive_type_id
               )
               const currentSourceType = responseSourceTypes.data.data.find(
                  (item) => item.source_type_id === responseData.source_type_id
               )
               const currentContractType = responseContractTypes.data.data.find(
                  (item) =>
                     item.contract_type_id === responseData.contract_type_id
               )
               const currentTurnoverType = responseTurnoverTypes.data.data.find(
                  (item) =>
                     item.turnover_type_id === responseData.turnover_type_id
               )
               setGetProductDate(new Date(responseData.operation_date))
               setInvoiceDate(new Date(responseData.doc_date))
               setTurnoverTypes(currentTurnoverType)
               setContractTypes(currentContractType)
               setSourceTypes(currentSourceType)
               setCompany(currentCompany)
               setReceiveTypes(currentRecieveType)
            }
            setLoading(false)
         } catch (e) {
            setLoading(false)
   ...
   const onSubmit = async (e) => {
      ...
      try {
         const response = await axios.put('/nakls-full/' + data.nakl_id, nakl, {
            headers: { accessToken: auth.token },
         })
      ...
      <div className="edit">
         <div className="container">
            <form className="edit__block" onSubmit={onSubmit}>
               <div className="edit__block-title">Редактирование накладной</div>
               <div className="edit__block-form">
                  ...
                  <div className="edit__block-form-item">
                     <div className="edit__block-form-left">
                        {form.formControls.invoiceNumber.label}
                     </div>
                     <div className="edit__block-form-right">
                        <input
                           type={form.formControls.invoiceNumber.type}
                           value={form.formControls.invoiceNumber.value}
                           required
